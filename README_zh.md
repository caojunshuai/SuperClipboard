...
### 编辑与模板
- **内联文本编辑器** — 右键文本卡片 → 编辑，Ctrl+Enter 保存，保存时自动跨行去重
- **固定模板** — 5 个预设模板（邮件签名、问候语、函数模板、表格模板、快递地址）+ 自定义；点击复制时自动替换 {date}/{time} 占位符
...
│       ├── CardList.tsx          # 可滚动卡片列表，含动效和键盘导航
│       ├── SearchBar.tsx         # 搜索框和类型/日期筛选
│       ├── DatePicker.tsx        # 自定义日历下拉组件
│       ├── TabBar.tsx            # 全部 / 收藏标签页 + 来源筛选
│       ├── HotkeyInput.tsx       # 快捷键捕获输入组件
│       ├── TemplateList.tsx      # 模板列表：新建、编辑、删除、复制
